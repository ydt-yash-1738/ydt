// // Hexagon movement and gyroscopic effect for sm devices
...
//   // Request permission on user interaction for iOS
...
//         // For iOS 13+ we need to request permission on user interaction
//         if (typeof DeviceOrientationEvent.requestPermission === 'function') {
//           // Don't automatically request here, wait for user interaction
//           console.log('iOS device detected - permission request needed');
//         } else {
...
//     // Add touch event listener to request permission on first touch
//     const handleFirstTouch = () => {
//       if (isMobile && !isGyroscopeEnabled && !permissionRequested) {
...
//     };
//   }, [isMobile, isGyroscopeEnabled, permissionRequested]);
...
//       const now = Date.now();
//       if (now - lastGyroUpdate.current < 16) return; // Throttle to ~60fps
//       lastGyroUpdate.current = now;

//       const { beta, gamma } = event;

//       if (beta !== null && gamma !== null && beta !== undefined && gamma !== undefined) {
//         // Normalize the values with better sensitivity
...
//     const handleDeviceOrientationAbsolute = (event) => {
...
//         {/* Permission request overlay for iOS */}
//         {isMobile && !isGyroscopeEnabled && !permissionRequested && (
//           <div className="absolute top-4 left-4 right-4 z-50 bg-cyan-900/20 backdrop-blur-sm border border-cyan-400/30 rounded-lg p-3">
//             <p className="text-xs text-cyan-300 text-center">
//               Tap anywhere to enable motion effects
//             </p>
//           </div>
//         )}
...
//                   {/* Center content with motion effects */}
...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    // Set viewport height for Safari iOS
    const setVH = () => {
  ...
  // Enhanced iOS detection and permission request
  const requestMotionPermission = async () => {
    console.log('Requesting motion permission...');
    
    if (permissionAttempted.current) {
      console.log('Permission already attempted');
      return;
    }
  ...
  useEffect(() => {
    const checkDevice = () => {
      ...
      console.log('Device check:', { isMobileDevice, isIOSDevice });

      if (isMobileDevice && typeof DeviceOrientationEvent !== 'undefined') {
        // For non-iOS devices or older iOS versions without permission requirement
        if (!isIOSDevice || typeof DeviceOrientationEvent.requestPermission !== 'function') {
          console.log('Enabling gyroscope without permission request');
          setIsGyroscopeEnabled(true);
        }
      }
    };
  ...
  // Enhanced touch/click handler for iOS permission
  useEffect(() => {
    const handleUserInteraction = async (event) => {
      console.log('User interaction detected:', event.type);
      
      if (isMobile && !isGyroscopeEnabled && !permissionAttempted.current) {
        console.log('Attempting to request permission...');
  ...
  // Enhanced gyroscope handler
  useEffect(() => {
    if (!isMobile || !isGyroscopeEnabled) {
      console.log('Gyroscope not enabled:', { isMobile, isGyroscopeEnabled });
      return;
    }

    console.log('Setting up gyroscope listeners...');
    ...
    const handleDeviceOrientation = (event) => {
      ...
      console.log('Gyro data:', { beta, gamma, alpha });

      if (beta !== null && gamma !== null && beta !== undefined && gamma !== undefined) {
        // Enhanced normalization for better sensitivity
        let normalizedX = Math.max(-1, Math.min(1, gamma / 30)); // Reduced divisor for more sensitivity
        let normalizedY = Math.max(-1, Math.min(1, (beta - 45) / 30)); // Reduced divisor
    ...
    // Try both event types for better compatibility
    window.addEventListener('deviceorientation', handleDeviceOrientation, { passive: true });
    ...
    window.addEventListener('deviceorientation', testHandler, { once: true, passive: true });
  ...
  // Calculate transform position with enhanced sensitivity
  const transformPosition = isMobile && isGyroscopeEnabled
    ? { x: gyroscopeData.x * 80, y: gyroscopeData.y * 80 } // Increased multiplier for more noticeable effect
    : !isMobile && window.innerWidth >= 1024
      ? mousePosition
      : { x: 0, y: 0 };
  ...
  return (
    <>
      <PortfolioStyles />
      <div className="h-screen w-screen bg-black text-white relative overflow-hidden safari-fix chrome-mobile-fix ios-scroll-fix" 
           style={{ minHeight: 'calc(var(--vh, 1vh) * 100)' }}>
        
        {/* Enhanced permission request overlay for iOS */}
        {isMobile && !isGyroscopeEnabled && !permissionRequested && (
          <div className="absolute top-4 left-4 right-4 z-50 bg-cyan-900/20 backdrop-blur-sm border border-cyan-400/30 rounded-lg p-3">
            <p className="text-xs text-cyan-300 text-center">
              {isIOS ? 'Tap anywhere to enable motion effects for iOS' : 'Tap anywhere to enable motion effects'}
            </p>
          </div>
        )}
        ...
        {/* {isMobile && (
          <div className="absolute top-20 left-4 z-50 text-xs text-cyan-400 bg-black/50 p-2 rounded">
            ...
            <div>Permission Requested: {permissionRequested ? 'Yes' : 'No'}</div>
            <div>X: {gyroscopeData.x.toFixed(2)}, Y: {gyroscopeData.y.toFixed(2)}</div>
            <div>Transform: {transformPosition.x.toFixed(1)}, {transformPosition.y.toFixed(1)}</div>
          </div>
        )} */}
        ...
        <div className="relative z-10 h-full w-full px-3 sm:px-4 lg:px-6 flex items-center chrome-mobile-fix py-4 lg:py-8" 
             style={{ minHeight: 'calc(var(--vh, 1vh) * 100)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 items-center w-full max-w-7xl mx-auto">
