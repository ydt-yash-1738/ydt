...
//         const normalizedX = Math.max(-1, Math.min(1, gamma / 45)); // Increased sensitivity
//         const normalizedY = Math.max(-1, Math.min(1, (beta - 45) / 45)); // Increased sensitivity
...
// Portfolio with 3D tilt effect for mobile devices only
import React, { useState, useEffect, useRef } from 'react';
import { Mail, Linkedin, Github, ExternalLink, Code } from 'lucide-react';
...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    const handleDeviceOrientation = (event) => {
      ...
      if (beta !== null && gamma !== null && beta !== undefined && gamma !== undefined) {
        ...
        // Calculate 3D tilt values for mobile
  ...
  // Calculate transform position for hexagon element (unchanged for desktop)
  const transformPosition = isMobile && isGyroscopeEnabled
    ? { x: gyroscopeData.x * 60, y: gyroscopeData.y * 60 }
    : !isMobile && window.innerWidth >= 1024
      ? mousePosition
      : { x: 0, y: 0 };

  // Mobile 3D tilt transform
  const mobileTiltTransform = isMobile && isGyroscopeEnabled
    ? `perspective(1000px) rotateX(${tiltValues.rotateX}deg) rotateY(${tiltValues.rotateY}deg)`
    : 'none';
  ...
  return (
    <>
      <PortfolioStyles />
      <div 
        className={`h-screen w-screen bg-black text-white relative overflow-hidden chrome-mobile-fix ${isMobile ? 'mobile-tilt-container' : ''}`} 
        style={{ 
          minHeight: '100dvh',
          transform: mobileTiltTransform
        }}
      >
        {/* Permission request overlay for iOS */}
        {isMobile && !isGyroscopeEnabled && !permissionRequested && (
        ...
        <div className={`relative z-10 h-full w-full px-3 sm:px-4 lg:px-6 flex items-center chrome-mobile-fix py-4 lg:py-8 ${isMobile ? 'mobile-tilt-inner' : ''}`} style={{ minHeight: '100dvh' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 items-center w-full max-w-7xl mx-auto">
